...
def optimize_dimension(args):
    ...
    for graph in G:
        ...
        H_s1 = NDE.structure_entropy(adj, node_list)

        adj_2 = adj.dot(adj)
        H_s2 = NDE.structure_entropy(adj_2, node_list)

        adj_3 = adj_2.dot(adj)
        H_s3 = NDE.structure_entropy(adj_3, node_list)

        p_list = NDE.assign_prob(H_s1, [H_s1, H_s2, H_s3])
        H_s =  NDE.align_entropy([adj,adj_2,adj_3],p_list,node_list)
    ...
    #find one dimension center
    kmeans_1 = KMeans(n_clusters=1)
    kmeans_1.fit(np.array(dimension).reshape(-1, 1))
    centers = kmeans_1.cluster_centers_
    onek_1 = int(list(centers.flatten())[0])
...
def task_val(args,num_classes, dim0, dim1, dim2, G1, G2):
    device = args.device
    all_vals = []
    duration = 0
    for fold_idx in range(10):
        ...
        """
        model = GraphCNN(args.num_layers, args.num_mlp_layers, train_G1[0].node_features.shape[1],  dim0, dim1, dim2,
        ...
        total_test = len(test_G1) + len(test_G2)
        for epoch in range(1, args.epochs + 1):
            scheduler.step()
            test_correct = 0
            
            for train_graphs,test_graphs,road in zip([train_G1,train_G2],[test_G1,test_G2],[0,1]):
...
def main():
    ...
    acc = task_val(args, num_classes, dim1_1, dim2_1, dim2_2, G1, G2)
